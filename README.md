# Stack-Overflow-Developer-Survey-Analysis
capstone project data
# Stack Overflow Developer Survey Analysis

This project analyzes technology trends among developers using the Stack Overflow Developer Survey dataset.

## Dashboard

Interactive dashboard created using IBM Cognos Analytics.

Dashboard link:
https://eu2.ca.analytics.ibm.com/bi/?perspective=dashboard&pathRef=.my_folders%2FDashboard%2Bview%2Bof%2BDashboard%2BStack%2BOverflow&action=view&mode=dashboard&nav_filter=true

## Dashboard PDF

Download the PDF:

[Stack Overflow Survey PDF](Dashboard_Stack_Overflow.pdf)
